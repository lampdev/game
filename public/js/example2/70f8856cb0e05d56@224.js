...
let loop = prompt('Какая группа?', 'K3');

export default function define(runtime, observer) {
  const main = runtime.module();
  const fileAttachments = new Map([["suits.csv",new URL("./files/"+loop,import.meta.url)]]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  main.variable(observer()).define(["md"], function(md){return(
md`# Deriyenko Ivan's quasigroups `
)});
  ...
  main.variable(observer("chart")).define("chart", ["data","d3","width","height","types","color","location","drag","linkArc","invalidation"], function(data,d3,width,height,types,color,location,drag,linkArc,invalidation)
{
  ...
  const svg = d3.create("svg")
      .attr("viewBox", [-width / 2 - 100, -height / 2 - 100, width + 100, height + 150])
